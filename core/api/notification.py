...
def create_notification(request: HttpRequest):
    ...
    :param request:
        ...
        to_user: to user pk
        micro_knowledge: micro_knowledge pk
    :return:
    """
    ...
    try:
        ...
        if params.get('micro_knowledge', None):
            mk = MicroKnowledge.objects.get(pk=params.get('micro_knowledge', None))
        else:
            mk = None
        to_user = User.objects.get(pk=params.get('to_user', None))
    except Exception:
        return  failed_api_response(ErrorCode.INVALID_REQUEST_ARGS, "bad params")
    if Notification.new_notification(code=code, from_user=from_user, to_user=to_user, content=content, micro_knowledge=mk):
